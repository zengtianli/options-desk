...
## 2026-08-29 第五轮 · 把两件「等用户」的也去穷尽了

上一轮结尾我写「真正等你的还是那两件」。**那时我还没真去试。** 这一轮各试一遍，拿证据说话。

### 一、06-18 的 $35,939.30 —— 证据升级，但仍定不了「去哪了」

拿 MCP 权威侧查（不是从库里推）：

| 查什么 | 结果 |
|---|---|
| `get_realized_pnl(700013444, 2026-06-01..06-30)` | 整个六月**只有 06-18 有成交**，`realized_gain = -2,686`，11 笔，`-11.03%` |
| `get_pnl_trade_history(700013444, span=all)` | 全历史就那 11 笔期权平仓（06-18 12:05~12:11 ET），`next_cursor` 空，**无任何 transfer 行** |
| 该账户当天 NLV | 60,004 → 21,374（约 −38,630） |

**结论 1（新）：它确定是现金走掉的，不是亏掉的。** 交易只解释 $2,686，剩下 ≈$35,944
与那笔 −$35,939.30 对得上（差约 $5 = mark 噪音）。

**结论 2（新）：该账户整本账闭合。**
入 10,000（期初）+ 50,000（06-11 自 5UK）= 60,000；
出 35,939.30 + 21,373.90 = 57,313.20；交易亏 2,686 →
`60,000 − 57,313.20 − 2,686 = 0.80` ≈ 残值 0.06。

**仍定不了的：去哪了。** 而这恰恰是合并总账唯一在乎的事——
内部划转 → 当天净流 0；外部提现 → 当天净流 −35,939.30。
5UK 同窗未解释 +620,801.63 里主体是那笔约 700 股 QQQ 的指派现金，而六月的指派记录与
06-17 前的股票单在库里都不存在，**那 62 万本身带着未知大小的窟窿**，3.6 万剥不出来。

> **为什么不只写一边**：若只给 700013444 记 −35,939.30 而不给 5UK 记 +35,939.30，
> 合并总账就变成 −35,939.30 —— 那等于**断言**它是外部提现。所以两边都不写。

MCP 确实没有 transfers/deposits/statements 接口 —— 这次是**逐个加载**
`get_accounts` / `get_portfolio` / `get_realized_pnl` / `get_pnl_trade_history`
四个工具的真 schema 确认的，不是搜索名字得出的推断。
→ 仍需 2026-06 月结单的 Transfers 段，或 App → Account → Transfers 看 06-18~06-19。

### 二、装真机 —— 手机当前不可达

```
$ xcrun devicectl list devices          # 5 台
  simulated | iPhone 17     | disconnected | paired
  physical  | iPhone 17     | unavailable  | paired     ← 真机在这
  simulated | iPhone 17 Pro | connected    | paired
  ...
$ detect_device.py  →  rc=1（挑不出可用真机）
```

真机**已配对**但 `tunnelState: unavailable` —— 没连线 / 不在同一网络 / 锁屏。
...
## 附：口径坑清单（2026-08-29 从 `CLAUDE.md` 搬来）

搬的理由是**常驻预算**：这两张表全部已 ✅，属于「怎么修好的」的证据，不是每会话都要看的判断。
CLAUDE.md 里留了一段三行摘要 + 指回这里。
...
## 已了结的取证（2026-08-30 从 CLAUDE.md 搬入）

> 这三段的结论已经写进库、写进 API、写进 app，不再需要每次会话都载入常驻上下文。
> 搬这里是为了常驻预算，**不是因为它们过时了** —— 每一条都仍然成立。
