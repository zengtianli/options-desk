...
```
700013444    2026-06-09  nlv = 10,000.00
             2026-06-22  nlv =      0.06     ← 此后 77 行一直 0.06,至 2026-08-27
             net_deposit 非零行数 = 0        ← 那一万块的去向零记录
```

合并 TWR 时如果不把这笔当 CONTRIBUTION/WITHDRAWAL 处理，链接收益率里会挂一个假的
接近 -100% 的单日。权重极小（$10k vs $1.36M，实测对累计的影响 1.2pp）但性质是**脏数据不是噪音**。
**是内部划转（净额为零、不该计入任一侧）还是提现（合并账当日记 -10000）——结果不同，必须靠证据定，禁猜。**
