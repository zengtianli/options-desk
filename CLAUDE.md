...
```
区间      2026-06-09 → 2026-08-27   49 个链接日,净入金 $115,570
TWR 累计  +23.35%
QQQ 同窗  +1.88%
★ 超额    +21.48pp        ← 回填之前这一栏根本算不出来
...
超额**扛得住风险调整**（beta 0.83 / 波动比 1.46x / 波动配平后仍 +20.6pp），但别当能力证据：
Sharpe 3.07 **± 2.29**（两倍标准误跨过 0），相关只有 0.57，且这 49 天 QQQ 净走 +1.88%
中途下探 693 —— 正是卖方结构最舒服的震荡市。**这是 regime-conditional 的结果，不是可外推的 edge。**
...
| 序列太短 | `rh_history` 从 **2026-06-09** 起，49 个链接日 | ⛔ **无解，只能等**。处理方式＝不在首屏放需要长样本的数（年化/Sharpe/回撤） |
| `net_deposit` 稀疏 | 主账户 2 行非零（合计 $115,570）；**第二账户 `700013444` 从 $10,000 掉到 $0.06 而零记录** | 🔧 进行中。那笔 $10k 是内部划转还是提现，两种处理结果不同，**必须靠证据定，禁猜** |
