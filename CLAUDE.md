...
```
区间      2026-06-09 → 2026-08-27   49 个链接日,已记录净入金 $115,570
TWR 累计  +23.35%   ← ⚠ 暂定值,见下
QQQ 同窗  +1.88%
...
⚠ **这个 +23.35% 是暂定值,不是结论。** 50 个交易日里 **17 天的 `net_deposit` 是 NULL**
...
| 情景 | 累计 | 超额 | n |
|---|---|---|---|
| 未知现金流全当 0(上表口径) | +23.35% | +21.48pp | 49 |
| 06-18 那笔 $35,939 按**提现**计 | +27.66% | +25.78pp | 49 |
| 起点挪到 06-23(两笔悬案都在起点前) | +10.51% | +9.46pp | 42 |
...
| 交易日数 | `rh_history` 每户 77 行但只有 **50 个不同日期**；缺 06-10/06-17/06-29/07-06/07-08/07-30/08-04 共 7 个交易日 | 直接是 TWR 的分母。原来写的「53 个交易日」与库对不上 |
| 同日多快照 | 13 个日子有多条（07-31 一天 9 条；06-25 的 16:00 与 16:14 两条 nlv 只差 26.60 而 cash 差 497,300） | 日粒度必须取「当日 UTC 最晚一条」，裸 `substr(asof,1,10)` 会挑错行 |
| `rh_orders` 恰好 1000 行 | `COUNT = COUNT DISTINCT order_id = 1000`，七月仅 92 条 | 典型 API 分页上限特征。**所有基于 net_cash 的对账都建在「订单表完整」这个未验证假设上** |
